fprintf('\n\tSoluciones de sistemas por medio de Rungue kutta 4\n')
...
for i=1:n 
    k1=h*f1(t1,w);
    k2=h*f1(t1+h/2,w+k1/2);
    k3=h*f1(t1+h/2,w+k2/2);
    k4=h*f1(t1+h,w+k3);
...
legend('SOL APROX','SOL EXACTA')
              ...
              W=[W,w];
              T=[T t1];
fprintf(' \t\t      MÉTODO DE RK4 PARA SISTEMA DE ORDEN 1\n\n');
fprintf('\t------------------------\n');
fprintf('\t t(i)\t     w(ti) \t  \n');
fprintf('\t-------------------------\n');
 fprintf('\t %3.1f\t      %3.7f\n',t1,w)
...
x2=x20;
W2=[W2 x2];
fprintf('\n\t\tMÉTODO DE RK4 PARA SISTEMAS DE ORDEN 2  \n\n');
fprintf('\t\t\t---------------------------------------------\n');
...
x3=x30;
W3=[W3 x3];
fprintf('\n\t\t MÉTODO DE RK4 PARA SISTEMAS DE ORDEN 3 \n\n');
fprintf('\t------------------------------------------------------\n');
...
for i=1:n
    ...
    x1=x1+(k(1,1)+2*k(2,1)+2*k(3,1)+k(4,1))/6;
    x2=x2+(k(1,2)+2*k(2,2)+2*k(3,2)+k(4,2))/6;
    x3=x3+(k(1,3)+2*k(2,3)+2*k(3,3)+k(4,3))/6;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    t1=a+i*h;
    W1=[W1,x1];  
    W2=[W2,x2];
...
x4=x40;
W4=[W4 x4];
fprintf(' \t\t MÉTODO DE RK4 PARA SISTEMAS DE ORDEN 4 \n\n');
fprintf('\t------------------------------------------------------\n');
...
for i=1:n
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    k(2,1)=h*f1(t1+h/2,x1+(1/2)*k(1,1),x2+(1/2)*k(1,2),x3+(1/2)*k(1,2),x4+(1/2)*k(1,2));
    k(2,2)=h*f2(t1+(h/2),x1+(1/2)*k(1,1),x2+(1/2)*k(1,2),x3+(1/2)*k(1,2),x4+(1/2)*k(1,2));
    k(2,3)=h*f3(t1+(h/2),x1+(1/2)*k(1,1),x2+(1/2)*k(1,2),x3+(1/2)*k(1,2),x4+(1/2)*k(1,2));
    k(2,4)=h*f4(t1+(h/2),x1+(1/2)*k(1,1),x2+(1/2)*k(1,2),x3+(1/2)*k(1,2),x4+(1/2)*k(1,2));
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    k(3,1)=h*f1(t1+(h/2),x1+(1/2)*k(2,1),x2+(1/2)*k(2,2),x3+(1/2)*k(2,3),x4+(1/2)*k(2,4));
    k(3,2)=h*f2(t1+(h/2),x1+(1/2)*k(2,1),x2+(1/2)*k(2,2),x3+(1/2)*k(2,3),x4+(1/2)*k(2,4));
    k(3,3)=h*f3(t1+(h/2),x1+(1/2)*k(2,1),x2+(1/2)*k(2,2),x3+(1/2)*k(2,3),x4+(1/2)*k(2,4));
    k(3,4)=h*f4(t1+(h/2),x1+(1/2)*k(2,1),x2+(1/2)*k(2,2),x3+(1/2)*k(2,3),x4+(1/2)*k(2,4));
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    k(4,1)=h*f1(t1+h,x1+k(3,1),x2+k(3,2),x3+k(3,3),x4+k(3,4));
    k(4,2)=h*f2(t1+h,x1+k(3,1),x2+k(3,2),x3+k(3,3),x4+k(3,4));
    k(4,3)=h*f3(t1+h,x1+k(3,1),x2+k(3,2),x3+k(3,3),x4+k(3,4));
    k(4,4)=h*f4(t1+h,x1+k(3,1),x2+k(3,2),x3+k(3,3),x4+k(3,4));
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
    x1=x1+(k(1,1)+2*k(2,1)+2*k(3,1)+k(4,1))/6;
    x2=x2+(k(1,2)+2*k(2,2)+2*k(3,2)+k(4,2))/6;
    x3=x3+(k(1,3)+2*k(2,3)+2*k(3,3)+k(4,3))/6;
    x4=x4+(k(1,4)+2*k(2,4)+2*k(3,4)+k(4,4))/6;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    t1=a+i*h;
    W1=[W,x1];   
    W2=[W2,x1];
